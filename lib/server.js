...
var logger = log4js.getLogger('wstun:server');
  ...
  module.exports = wst_server = (function() {
    ...
    function wst_server(options) {
      ...
      if(options != undefined) {
        ...
        logger.trace("[SYSTEM] - WS Tunnel Server starting with these paramters:\n" + JSON.stringify(options, null, "\t"));
        ...
        logger.trace("[SYSTEM] - WS Tunnel Server starting...");

      if(https_flag == "true"){

        //HTTPS
        logger.trace("[SYSTEM] - WS over HTTPS");
        ...
        //HTTP
        logger.trace("[SYSTEM] - WS over HTTP");
    ...
    wst_server.prototype.start = function(port) {
      ...
      if (https_flag == "true")
        logger.trace("[SYSTEM] - WS Tunnel Server starting on: wss://localhost:" + port + " - CERT: \n" + this.s4t_cert);
      else
        logger.trace("[SYSTEM] - WS Tunnel Server starting on: ws://localhost:" + port);
      
      this.httpServer.listen(port, function() {
        return logger.trace("[SYSTEM] - Server is listening on port " + port + "...");
      });
      ...
      return this.wsServer.on('request', (function(_this) {
        return function(request) {
          ...
          tcpconn = net.connect( { port: port, host: host }, function() {
              var wsconn;
              logger.trace("[SYSTEM] - Establishing tunnel to " + remoteAddr);
    ...
    wst_server.prototype._reject = function(request, msg) {
      request.reject();
      return logger.trace("[SYSTEM] - Connection from " + request.remoteAddress + " rejected: " + msg);
    };
