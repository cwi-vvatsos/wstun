...
var logger = log4js.getLogger('wstun:server_reverse');
...
wst_server_reverse = function(options) {
  ...
  if(options != undefined) {
    ...
    logger.trace("[SYSTEM] - WS Reverse Tunnel Server starting with these paramters:\n" + JSON.stringify(options, null, "\t"));
    ...
    logger.trace("[SYSTEM] - WS Reverse Tunnel Server starting...");


  if(https_flag == "true"){
    
    //HTTPS
    logger.trace("[SYSTEM] - WS Reverse Tunnel Server over HTTPS.");
    ...
    //HTTP
    logger.trace("[SYSTEM] - WS Reverse Tunnel Server over HTTP.");
...
wst_server_reverse.prototype.start = function(port) {
  ...
  if (https_flag == "true")
    logger.trace("[SYSTEM] - WS Reverse Tunnel Server starting on: wss://localhost:" + port + " - CERT: \n" + this.s4t_cert);
  else
    logger.trace("[SYSTEM] - WS Reverse Tunnel Server starting on: ws://localhost:" + port);

  //Activate HTTP/S server
  this.httpServer.listen(port, function() {
    logger.trace("[SYSTEM] - WS Reverse Tunnel Server is listening...");
  });
  ...
  this.wsServerForControll.on('request', (function(_this){
    return function(request){
      ...
      if (uri.query.dst != undefined){
        ...
        logger.trace("[SYSTEM] WebSocket creation towards " + src_address + " on port " + portTcp );
        
        request.tcpServer.listen(portTcp);
        logger.trace("[SYSTEM] --> TCP server is listening on port " + portTcp);

        request.wsConnectionForControll = request.accept('tunnel-protocol', request.origin);
        logger.trace("[SYSTEM] --> WS connection created");

        request.wsConnectionForControll.on('close', function(reasonCode, description) {
          logger.trace("[SYSTEM] - WebSocket Controll Peer " + request.wsConnectionForControll.remoteAddress + " disconnected - Reason: ["+reasonCode+"] " + description);
          logger.trace("[SYSTEM] --> Close websocket server on port " + portTcp);
        ...
        logger.trace("[SYSTEM] --> WebSocket Request for Data");
      ...
      request.tcpServer.on('error', function(message) {
        ...
        if(message.code == "EADDRINUSE"){
          logger.trace("[SYSTEM] - Error - Port " + message.port + " already used: connection aborted.");
          request.wsConnectionForControll.close();
        }else
          logger.trace("[SYSTEM] - Error establishing TCP connection: " + message);
