...
var logger = log4js.getLogger('wstun:bindSocket');
...
(function() {
  ...
  module.exports = bindSockets = function(wsconn, tcpconn) {
    ...
    wsconn.on("error", function(err) {
      return logger.trace('ws Error ' + err);
    });
    
    wsconn.on('close', function(reasonCode, description) {
      logger.trace("[SYSTEM] --> WS Peer " + wsconn.remoteAddress + ' disconnected - Reason: '+description);
    ...
    tcpconn.on("error", function(err) {
      return logger.trace('tcp Error ' + err);
    });
